...
class ModelSaleDpdfrance extends Model {
    public function addOrder($data) {
        ...
        if (!empty($this->request->post['affiliate_id'])) {
            ...
            $affiliate_info = $this->model_marketing_affiliate->getAffiliate($this->request->post['affiliate_id']);
    ...
    public function getCustomer($customer_id){
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customer` c WHERE c.customer_id = '" . (int)$customer_id . "'");
        return $query->rows[0];
    }
    ...
    public function getOrders($data = array()) {
        $sql = "SELECT o.order_id, CONCAT(o.shipping_firstname, ' ', o.shipping_lastname) AS customer, o.shipping_country_id, SUBSTR(o.shipping_method,22) AS relais_address, CONCAT(o.shipping_address_1, ' ', o.shipping_postcode, ' ' , o.shipping_city) AS address, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.total, o.shipping_code, o.currency_code, o.currency_value, o.date_added, o.date_modified, o.jadlog_embarcador_response_or_status, o.jadlog_embarcador_dfes FROM `" . DB_PREFIX . "order` o";

        if (isset($data['filter_order_status_id']) && !is_null($data['filter_order_status_id'])) {
            $sql .= " WHERE o.shipping_code LIKE 'jadlog%' AND o.order_status_id = '" . (int)$data['filter_order_status_id'] . "'";
        } else {
            $sql .= " WHERE o.shipping_code LIKE 'jadlog%' AND o.order_status_id NOT IN (0,3,7)";
        }
    ...
    public function saveMsgEmbarcador($order_id, $msg) {
        $query = $this->db->query("update " . DB_PREFIX . "order set jadlog_embarcador_response_or_status = '".$this->db->escape($msg)."' WHERE order_id = '" . (int)$order_id . "'");
    }
    public function saveEmbarcadorDfes($order_id, $dfespost) {
        $query = $this->db->query("update " . DB_PREFIX . "order set jadlog_embarcador_dfes = '".$this->db->escape($dfespost)."' WHERE order_id = '" . (int)$order_id . "'");
    }
    public function saveEmbarcadorTrackingNumber($order_id, $trackingNumber){
        $query = $this->db->query("update " . DB_PREFIX . "order set jadlog_embarcador_tracking_number = '".$this->db->escape($trackingNumber)."' WHERE order_id = '" . (int)$order_id . "'");
    }
    ...
    public function addOrderHistory($order_id, $data) {
        ...
        if ($data['notify']) {
            ...
            $language->load('sale/dpdfrance');

            $subject = sprintf($language->get('text_subject'), $order_info['store_name'], $order_id);

            $message  = $language->get('text_order') . ' ' . $order_id . "\n";
            $message .= $language->get('text_date_added') . ' ' . date($language->get('datetime_format'), strtotime($order_info['date_added'])) . "\n\n";
